 ...
 */
export async function eventsGetTargetsDeprecated(
  client: SDKCore,
  workspaceId: string,
  ...
  >
> {
  const input: operations.GetWorkspaceTargetsDeprecatedRequest = {
  ...
  const parsed = safeParse(
    input,
    (value) =>
      operations.GetWorkspaceTargetsDeprecatedRequest$outboundSchema.parse(
        value,
      ),
  ...
  const path = pathToFunc("/v1/workspace/{workspace_id}/events/targets")(
    pathParams,
  );
  ...
  >(
    M.json(
      200,
      operations.GetWorkspaceTargetsDeprecatedResponse$inboundSchema,
      { key: "TargetSDKList" },
    ),
    M.fail("4XX"),
