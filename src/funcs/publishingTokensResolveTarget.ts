 ...
 */
export function publishingTokensResolveTarget(
  client: SDKCore,
  tokenID: string,
...
> {
  const input: operations.GetPublishingTokenTargetByIDRequest = {
    tokenID: tokenID,
  };
  ...
  const parsed = safeParse(
    input,
    (value) =>
      operations.GetPublishingTokenTargetByIDRequest$outboundSchema.parse(
        value,
      ),
  ...
  const path = pathToFunc("/v1/publishing-tokens/{tokenID}/target")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.text(200, operations.GetPublishingTokenTargetByIDResponse$inboundSchema, {
      key: "res",
    }),
