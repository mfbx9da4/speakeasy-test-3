 ...
 */
export function subscriptionsIgnoreSubscriptionNamespace(
  client: SDKCore,
...
> {
  const input: operations.IgnoreSubscriptionNamespaceRequest = {
  ...
  const parsed = safeParse(
    input,
    (value) =>
      operations.IgnoreSubscriptionNamespaceRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    namespaceName: encodeSimple("namespaceName", payload.namespaceName, {
      explode: false,
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.nil("2XX", operations.IgnoreSubscriptionNamespaceResponse$inboundSchema),
