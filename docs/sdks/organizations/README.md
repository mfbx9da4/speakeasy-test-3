...
async function run() {
  const result = await sdk.organizations.deleteBillingAddOn(BillingAddOn.SnippetAi);
...
async function run() {
  const res = await organizationsDeleteBillingAddOn(sdk, BillingAddOn.SnippetAi);
