...
async function run() {
  const res = await authGetAccessToken(sdk, "<value>");
...
```typescript
import { SDKCore } from "petstore/core.js";
import { authGetWorkspaceAccess } from "petstore/funcs/authGetWorkspaceAccess.js";
...
async function run() {
  const res = await authGetWorkspaceAccess(sdk);
